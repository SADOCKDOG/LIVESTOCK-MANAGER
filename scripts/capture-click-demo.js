/**
 * Captura - Click en "Cargar Demo" primero, luego navega
 */
...
async function captureScreenshots() {
  console.log('🚀 Captura - Click Demo + Navegar\n');
  ...
  try {
    ...
    // Hacer click en "Cargar Demo CHAMORRO"
    console.log('📦 Buscando botón demo...');
    await page.evaluate(() => {
      const buttons = Array.from(document.querySelectorAll('button, [role="button"]'));
      const demoBtn = buttons.find(b => b.textContent.includes('Cargar Demo'));
      if (demoBtn) {
    ...
    for (let i = 0; i < screenshots.length; i++) {
      ...
      try {
        // Navegar
        await page.goto(BASE_URL + ss.path, { waitUntil: 'networkidle0', timeout: 10000 });
