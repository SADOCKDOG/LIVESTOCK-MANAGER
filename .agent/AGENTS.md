...
- **Iconografía (REGLA ESTRICTA, 2026-07-03)**: PROHIBIDO el uso de emoticonos/pictogramas Unicode en cualquier string visible (UI, PDF, print, placeholders) y en la documentación. Todo icono se resuelve con la librería SVG `Icons.*` (`js/icons.js`), dimensiona con `.icon` (1.2em, escala con el `font-size` del contenedor) y hereda `currentColor`. Se permiten símbolos tipográficos (✓ ✕ ✗ ♀ ♂ ➔ ← ·). Los toasts declaran su tipo de forma EXPLÍCITA: `App.toast(msg, 'success'|'error'|'warning'|'info')` — prohibido codificar el tipo con emojis en el mensaje (la inferencia por marcador pervive solo como fallback legacy para strings dinámicos).
...
- **Ficha de registro (layout aprobado v4.8)**: 2 columnas en flexbox con alineación `stretch`. Información principal a la izquierda con identificadores en **Oro (`var(--p-gold)`)** y peso **950**. A la derecha, un raíl con el **estado arriba en Viñeta Iluminada** (cápsula con resplandor neón `drop-shadow` y fondo al 15%) y la **acción "FICHA ➔" abajo** en color aviso (`var(--c-warning)`). Consultar `docs/PLANTILLA-CARD-REGISTRO.md` para el código exacto.
- **PLANTILLA DEFINITIVA de lista de registros** (patrón **Aglutinadora**): toda lista va precedida por una **card de RESUMEN** de los datos (`.card-resumen`) con botón de colapso y una fila de **filtros integrados** (búsqueda + selectores). No dejar listas de fichas "sueltas".
- Alineación: Información principal a la izquierda, estados (viñetas) y acciones (links) a la derecha.
...
- **Marco Principal completo (formato Cork Manager)**: (1) La cabecera se remata con una **línea neón fina y nítida** (`border-bottom: 1px` con `--header-neon-color`) más un **haz de luz difuso hacia abajo**; (2) **líneas laterales** de 1px con haz interior (overlay fijo `body::after`) que unen el borde inferior del header con el borde superior del bottom-nav; (3) el **bottom-nav** lleva la misma línea de 1px + haz hacia arriba en su borde superior. Header y bottom-nav comparten la **misma transparencia**: `rgba(18,18,18,0.4)` + `backdrop-filter: blur(12px)`.
- **REGLA TÉCNICA**: nunca incluir `border-color` en `transition` de elementos cuyo color viene de `var(--header-neon-color)` — `transition: all` + `var()` congela el color en el fallback (quirk Chromium/WebView).
- **Retroiluminación configurable**: El usuario configura el haz de luz desde el wizard de Retroiluminación (Ajustes → Sistema y Seguridad → Interfaz). Clases de estado que **todo glow nuevo debe respetar**: `body.glow-marco-off` (haz del marco/header+bottom-nav), `body.glow-laterales-off` (haz de las líneas laterales) y `body.glow-botones-off` (retroiluminación de botones y FAB).
- **Viñeta REGA en el header**: la viñeta derecha muestra el **código REGA** de la finca activa (no el nombre), en amarillo con haz de luz (`.rega-badge-container`). El header usa grid `1fr auto 1fr` (logo · píldora de ruta · viñeta REGA).
- **FAB (botón flotante)**: color dinámico heredado del módulo activo con memoria; intensidad y blanco retroiluminado configurables; respeta `glow-botones-off`. No crear FABs con color fijo — usar el sistema dinámico.
- **Configuración del sistema**: la pantalla de ajustes visuales/sistema es `ConfigSistemaView` (ruta `/sistema`, pestañas Interfaz/Seguridad/Auditoría). Las preferencias se guardan vía `AjustesView._saveConfig` (store `meta.appConfig`). No crear vistas de configuración paralelas.
