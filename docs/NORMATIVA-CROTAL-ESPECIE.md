...
- **Cambio pendiente de aplicar** (diagnosticado 2026-07-21, no implementado aún): en `js/error-handler.js` `CROTAL_FORMATOS` (línea ~307), añadir `equino_microchip: { regex: /^\d{15}$/, descripcion: "15 dígitos (UELN + correlativo, ISO 11784, ej. 724901000007790)" }`. En `js/db.js` `ESPECIE_TIPO_IDENTIFICADOR_SEED` (línea 56), cambiar `{ especieId: 5, tipoIdentificadorId: 14, formato: null }` por dos filas: mantener `tipoIdentificadorId: 14` (DIE) sin formato estricto (campo libre, por su variabilidad), y añadir `{ especieId: 5, tipoIdentificadorId: 3 (Inyectable electrónico), formato: 'equino_microchip' }` ya que el microchip es complementario obligatorio al DIE, no una alternativa.
...
## Catálogo de razas — gap de dato maestro sin cerrar (auditoría 2026-07-21)

**Estado actual del código**: `raza` es texto libre en el animal (`js/db.js:93-95`, input de texto en `js/views/animales-view.js:321`). Sin catálogo, sin FK a especie, sin validación.

**`Catálogo oficial de razas de ganado de España.csv`** (`RAZAS`/`CLASIFICACION_RAZAS`) — **189 razas reales**, columnas `Código;Raza;Especie;Clasificación;Grado de amenaza`, cubriendo Bovinos, Cerdos, Ovinos, Caprinos, Gallinas, Ocas, Conejos, Équidos, Dromedario. Complementado por `Clasificación en el catálogo oficial de razas de ganado de España.csv` (4 categorías: 1001 Raza Autóctona, 1002 Raza Autóctona Amenazada, 1003 Raza Integrada en España, 1004 Otras razas reconocidas en España).

Confirmado como campo obligatorio por el propio FEGA: `docs/AUDITAR/Catalogos_csv/20250514-Anexo_I_Definicion_de_Variables_Ganaderas_3.6.0.xlsx`, bloque "Datos individuales de los animales", exige junto a la raza: `Clasificación en el Catálogo Oficial de Razas`, `Asociación de criadores` (catálogo `Asociación de razas.csv`, ~170 entidades), `Resultado genotipado Scrapie` (28 combinaciones alélicas, obligatorio en vigilancia ovino/caprino), `Tipo de muerte` (solo 3 valores: Sacrificado/Muerto/Sacrificio de urgencia — más simple que el `MOTIVOS_BAJA` ya implementado en la app), y un campo `Morfotipo` que no tiene catálogo CSV correspondiente en la carpeta descargada (gap de la fuente, no de la app).

**⚠️ No confundir con el catálogo de razas propio del fichero de incorporación SIGGAN** (sección de arriba, "Tabla `Raza`") — ese es mucho más corto (solo caprino/ovino, ~15-20 valores) y usa **códigos numéricos distintos** a este catálogo general de 189 razas. Si se construye el exportador SIGGAN, hace falta tabla de correspondencia entre ambos, no asumir que son el mismo código.

**Propuesta de implementación** (mismo patrón que especies/tipos de identificador, no implementado aún): nueva tabla `razas` (código, nombre, especie_id FK a `especies`, clasificación, grado_amenaza), poblada desde el CSV ya descargado; convertir el input de texto libre de `raza` en un `<select>` filtrado por especie seleccionada. Prioridad alta recomendada — detalle de plan en [PLAN-MEJORA-SIGGAN.md](PLAN-MEJORA-SIGGAN.md).
