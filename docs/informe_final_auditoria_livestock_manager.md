...
| P0-4 | Lazy loading inicial: Convertir script src en script type=module con dynamic import() para vistas no-críticas | ⚠️ PARCIAL (2026-07-15): diferidas las 3 librerías CDN (xlsx.js ~700KB, html2pdf.js ~400KB, chart.js ~200KB — ~1.3MB en total) vía `App._ensureXLSX()`/`App._ensureHtml2Pdf()`/`App._ensureChartJs()`, cargando solo al exportar o al dibujar un gráfico. Verificado en vivo desde arranque en frío. Pendiente: el lazy-loading real de las ~90 vistas locales por ruta (requiere reescribir el router para inyectar scripts dinámicamente — cambio grande y de mayor riesgo, mejor en una sesión dedicada con más tiempo de pruebas). | index.html, js/app.js |
