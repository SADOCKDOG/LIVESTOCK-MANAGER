...
| P0-4 | Lazy loading inicial: Convertir script src en script type=module con dynamic import() para vistas no-críticas | ✅ COMPLETO (2026-07-15). Fase 1: diferidas las 3 librerías CDN (xlsx.js, html2pdf.js, chart.js — ~1.3MB) vía `App._ensureXLSX()`/`_ensureHtml2Pdf()`/`_ensureChartJs()`. Fase 2: lazy-loading real por pilar de las 25 vistas de ExPro/GeGan/CoMer/Informes + 6 periféricas, cargadas por grupo la primera vez que se visita una ruta de ese pilar vía un único punto de control en `App.route()` (`App._ensureRouteScripts`). Diseño por pilar completo (no archivo por archivo) para no romper la delegación interna entre vistas del mismo grupo. JS local síncrono en el arranque: 2.36MB → 960KB (-59%). Auditados todos los cruces app.js/wizards → vistas diferidas: casi todos ya degradaban con gracia (`if (window.X)` existente); 2 puntos de riesgo real corregidos con ensure explícito (Dashboard→Silos, wizard Venta Masiva→alta rápida de comprador). Verificado en vivo: arranque en frío sin ninguna vista cargada, las 25 rutas (incluido redirectMap) renderizan sin errores, cambio de pestaña dentro de cada pilar sin recargas. | index.html, js/app.js |
