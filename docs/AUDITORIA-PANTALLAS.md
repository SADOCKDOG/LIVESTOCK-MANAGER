...
- **2026-07-04** — Consolidación de estado (revisión post-multiagente). Trabajo integrado en master desde la auditoría: (1) **`.card-registro`** canónico con relieve/tipografía por tokens, adoptado en las listas de registro; (2) **navegación sin huérfanos** — el sheet «Más» es índice completo de módulos (Leche/Logística/etc. dejan de ser solo accesibles por el dropdown); (3) **banner REGA** — la viñeta muestra el código REGA con haz de luz; (4) **FAB personalizable** (color dinámico con memoria, intensidad, blanco retroiluminado) + **wizard de Retroiluminación** (marco/laterales/botones); (5) **refactor de Ajustes → `ConfigSistemaView`** (`/sistema`, pestañas Interfaz/Seguridad/Auditoría), −1125 líneas, `interfaz-view.js` (huérfano intermedio) eliminado; (6) **fix compra Premium** (restauración + `ITEM_ALREADY_OWNED`). Docs sincronizados (`ESTADO_PROYECTO.md`, `AGENTS.md`). Caché → `corcho-v6.10.34` / `styles.css?v=5.4.2`. **Ramas de agentes (`claude/*`, `copilot/verificar-estructura-siggan`, `sadockdog-*`): 0 commits únicos vs master → superadas; se dejan podables (worktrees bloqueados por permisos, se limpian con `git worktree prune` al liberarse).**
