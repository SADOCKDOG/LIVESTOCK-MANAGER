/**
 * Captura usando router.push() directamente
 */
...
async function captureScreenshots() {
  console.log('🚀 Captura con router.push()...\n');
  ...
  try {
    ...
    console.log('📦 Demo...');
    await page.evaluate(() => {
      window.SeedData && window.SeedData.run && window.SeedData.run();
    });
    ...
    for (let i = 0; i < screenshots.length; i++) {
      ...
      try {
        ...
        const result = await page.evaluate((routePath) => {
          if (window.__APP__ && window.__APP__.$router) {
            window.__APP__.$router.push(routePath);
            return 'pushed_app';
          } else if (window.$router) {
            window.$router.push(routePath);
            return 'pushed_window';
          } else {
